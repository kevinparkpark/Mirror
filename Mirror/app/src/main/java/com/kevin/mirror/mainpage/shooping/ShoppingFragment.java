...
import com.kevin.mirror.purchase.AllAddressesActivity;
        ...
        adapter.setClickListener(new FragmentToDetailsOnClickListener() {
            ...
            public void onFragmentToDetailsClickListener(int position) {
                if (!NetWorkStatus.isNetworkAvailable(MyApp.context)) {
                    ...
                    Intent intent = new Intent(context, AllAddressesActivity.class);
                    intent.putExtra("goods_name", dbBeen.get(position).getGoodsName());
                    intent.putExtra("goods_price", dbBeen.get(position).getGoodsPrice());
