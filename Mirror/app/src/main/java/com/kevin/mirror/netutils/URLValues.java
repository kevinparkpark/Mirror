package com.kevin.mirror.netutils;

/**
 * Created by kevin on 16/6/21.
 */
public final class URLValues {

    public static final String ADDADDR_URL = "http://api.mirroreye.cn/index.php/user/add_address";
    public static final String ADDR_URL = "http://api.mirroreye.cn/index.php/user/address_list";
    public static final String ALIPAY_URL = "http://api.mirroreye.cn/index.php/pay/ali_pay_sub";
    public static final String ALLKINDS_URL = "http://api.mirroreye.cn/index.php/index/mrtj";
    public static final String BUDDLING_URL = "http://api.mirroreye.cn/index.php/user/bundling";
    public static final String DELADDR_URL = "http://api.mirroreye.cn/index.php/user/del_address";
    public static final String EDITADDR_URL = "http://api.mirroreye.cn/index.php/user/edit_address";
    public static final String GOODSLIST_URL = "http://api.mirroreye.cn/index.php/products/goods_list";
    public static final String LOGIN_URL = "http://api.mirroreye.cn/index.php/user/login";
    public static final String MENULIST_URL = "http://api.mirroreye.cn/index.php/index/menu_list";
    public static final String MRADDRESS_URL = "http://api.mirroreye.cn/index.php/user/mr_address";
    public static final String ORDERSUB_URL = "http://api.mirroreye.cn/index.php/order/sub";
    public static final String ORDER_URL = "http://api.mirroreye.cn/index.php/order/order_list";
    public static final String REGISTER_URL = "http://api.mirroreye.cn/index.php/user/reg";
    public static final String SENDCODE_URL = "http://api.mirroreye.cn/index.php/user/send_code";
    public static final String SERVER_URL = "http://api.mirroreye.cn/";
    public static final String SPLASH_URL = "http://api.mirroreye.cn/index.php/index/started_img";
    public static final String STORYLIST_URL = "http://api.mirroreye.cn/index.php/story/story_list";
    public static final String STORYINFO_URL = "http://api.mirroreye.cn/index.php/story/info";
    public static final String WXPAY_URL = "http://api.mirroreye.cn/index.php/pay/wx_pay_sub";
    public static final String NUM="13504780339";
}
